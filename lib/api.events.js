	...
	[
		...
		function(req, res, next) {
			...
			Event.findOne(conditions).sort('-timestamp').exec(function(err, e) {
		        if (err || !e) return next(new Error("Error getting event"));
		        ...
		        EventModel.findOne({
			    	...
			    	if (err) return next(new Error("Error getting model"));
			    	...
			    	Event.count(conditions, function(err, count) {
			        	...
			        	res.jsonp({
				        	...
				        	'count': count,
				        	'properties': _.reduce(e.properties, function(memo, value, key) {
