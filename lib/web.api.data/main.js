...
function setup(options, imports, register) {
	var logger = imports.logger.namespace("api.data");
	var api = imports.api;
	...
	var processEvents = function(events, options) {
		var seriesMap = {}, serie = [];
		...
		if (!_.contains(_.keys(transformations), options.transform)) {
		...
		[
			...
			function(req, res, next) {
				...
				query.exec(function (err, events) {
					...
					var data = processEvents(events, req.query);
